...
static void parse_dsml_tool_calls(nc_chat_response *resp) {
    ...
    while ((invoke = strstr(invoke, "<｜｜DSML｜｜invoke name=\"")) != NULL) {
        ...
        while ((param = strstr(param, "<｜｜DSML｜｜parameter name=\"")) != NULL && param < end_invoke) {
            ...
            for (int i=0; i<val_len && args_len < (int)sizeof(args)-5; i++) {
...
#define CHAIN_MAX_RETRIES 3
#define CHAIN_RETRY_WAIT  10  /* seconds between retry loops */

    for (int attempt = 0; attempt < CHAIN_MAX_RETRIES; attempt++) {
        if (attempt > 0) {
            nc_log(NC_LOG_WARN, "Retry %d/%d in %ds...", attempt + 1, CHAIN_MAX_RETRIES, CHAIN_RETRY_WAIT);
        ...
        nc_log(NC_LOG_WARN, "Both providers failed (attempt %d/%d)", attempt + 1, CHAIN_MAX_RETRIES);
    }

    nc_log(NC_LOG_ERROR, "Provider chain exhausted after %d attempts", CHAIN_MAX_RETRIES);
    return false;
}
